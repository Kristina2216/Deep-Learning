...
class FC(Layer):
  ...
  def forward(self, inputs):
    ...
    """
    product=np.matmul(inputs, self.weights.transpose())
    self.inputs=inputs
  ...
  def backward_params(self, grads):
    ...
    """
    grad_weights = np.matmul(grads.transpose(),self.inputs)
...
class SoftmaxCrossEntropyWithLogits():
  ...
  def forward(self, x, y):
    ...
    """
    log_probs=-np.log(np.exp(x)/np.exp(x).sum(axis=1).reshape(-1,1))
    return np.mean((y*log_probs).sum(axis=1))
  ...
  def backward_inputs(self, x, y):
    ...
    probs=np.exp(x)/np.exp(x).sum(axis=1).reshape(-1,1)
    return (probs-y)/len(x)
...
class L2Regularizer():
  ...
  def forward(self):
    ...
    """
    return 0.5*self.weight_decay*(self.weights*self.weights).sum()
