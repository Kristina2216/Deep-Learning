...
def train(model, optimizer, loader, device='cuda'):
    ...
    for i, data in enumerate(loader):
        ...
        optimizer.zero_grad()
        loss = model.loss(Variable(anchor.to(device), requires_grad = True), Variable(positive.to(device), requires_grad = True), Variable(negative.to(device), requires_grad = True))
        loss.backward()
        optimizer.step()
